...
class UserProfile(AbstractBaseUser, PermissionsMixin):
    ...
    objects = UserProfileManager()  # 用户认证

    USERNAME_FIELD = 'email'  # 用email字段当作用户名
    ...
    class Meta:  # 自定义权限条目 对应auth_permission表
        permissions = (
            ('crm_table_list', '可以查看xadmin每张表里所有的数据'),
            ('crm_table_list_view', '可以访问xadmin表里每条数据的修改页'),
            ('crm_table_list_change', '可以对xadmin表里的每条数据进行修改'),
            ('crm_table_obj_add_view', '可以访问xadmin每张表的数据增加页'),
            ('crm_table_obj_add', '可以对xadmin每张表进行数据添加'),
...
class Student(models.Model):
    ...
    def __str__(self):
        return "%s" % self.customer  # 必须返回字符串
...
class StudentEnrollment(models.Model):
    ...
    contract_agreed = models.BooleanField(default=False)  # 学员报名
    contract_signed_date = models.DateTimeField(blank=True, null=True)
    contract_approved = models.BooleanField(default=False)  # 审批
